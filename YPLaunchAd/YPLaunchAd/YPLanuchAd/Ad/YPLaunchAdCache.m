...
@implementation YPLaunchAdCache
...
+(BOOL)saveImageData:(NSData *)data imageURL:(NSURL *)url{
    NSString *path = [NSString stringWithFormat:@"%@/%@",[self YPLaunchAdCachePath],[self keyWithURL:url]];
    if (data) {
        BOOL result = [[NSFileManager defaultManager] createFileAtPath:path contents:data attributes:nil];
        if (!result) YPLaunchAdLog(@"cache file error for URL: %@", url);
        return result;
    }
    return NO;
}
...
+(BOOL)saveVideoAtLocation:(NSURL *)location URL:(NSURL *)url{
    NSString *savePath = [[self YPLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
...
+ (NSString *)YPLaunchAdCachePath{
    NSString *path =[NSHomeDirectory() stringByAppendingPathComponent:@"Library/YPLaunchAdCache"];
...
+(NSString *)imagePathWithURL:(NSURL *)url{
    if(url==nil) return nil;
    return [[self YPLaunchAdCachePath] stringByAppendingPathComponent:[self keyWithURL:url]];
}

+(NSString *)videoPathWithURL:(NSURL *)url{
    if(url==nil) return nil;
    return [[self YPLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:url]];
}

+(NSString *)videoPathWithFileName:(NSString *)videoFileName{
    if(videoFileName.length==0) return nil;
    return [[self YPLaunchAdCachePath] stringByAppendingPathComponent:[self videoNameWithURL:[NSURL URLWithString:videoFileName]]];
}
...
+(void)clearDiskCache{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *path = [self YPLaunchAdCachePath];
        [fileManager removeItemAtPath:path error:nil];
        [self checkDirectory:[self YPLaunchAdCachePath]];
    });
}
...
+(void)clearDiskCacheExceptImageUrlArray:(NSArray<NSURL *> *)exceptImageUrlArray{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *allFilePaths = [self allFilePathWithDirectoryPath:[self YPLaunchAdCachePath]];
        ...
        YPLaunchAdLog(@"allFilePath = %@",allFilePaths);
    });
}
...
+(float)diskCacheSize{
    NSString *directoryPath = [self YPLaunchAdCachePath];
    BOOL isDir = NO;
...
+ (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        YPLaunchAdLog(@"create cache directory failed, error = %@", error);
    } else {
        [self addDoNotBackupAttribute:path];
    }
    YPLaunchAdLog(@"YPLaunchAdCachePath = %@",path);
}
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        YPLaunchAdLog(@"error to set do not backup attribute, error = %@", error);
    }
}
